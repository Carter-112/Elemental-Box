// Bubble Element
// Bubbles that float up, turn solid when cold, and pop when hot

const BubbleElement = {
    name: 'bubble',
    label: 'Bubble',
    description: 'Bubbles that float up, turn solid when cold, and pop when hot',
    category: 'gas',
    ...
    shouldPop: function(x, y, grid, isInBounds) {
        ...
        // Check for sharp objects, high pressure, or temperature that can affect the bubble
        const neighbors = [
            { dx: -1, dy: 0 }, { dx: 1, dy: 0 },
            { dx: 0, dy: -1 }, { dx: 0, dy: 1 },
            { dx: -1, dy: -1 }, { dx: 1, dy: -1 },
        ...
        let avgTemp = grid[y][x].temperature || 25;
        let tempCount = 1;
        ...
        grid[y][x].temperature = avgTemp;

        // Turn to solid when cold
        if (avgTemp < 0) {
            // Turn to solid bubble (glass-like)
            grid[y][x] = {
                type: 'glass',
                color: '#B0E0E6', // Light blue glass
                temperature: avgTemp,
                processed: true,
                isGas: false,
                isLiquid: false,
