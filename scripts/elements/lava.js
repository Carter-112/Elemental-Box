...
const LavaElement = {
    ...
    process: function(x, y, grid, isInBounds) {
        ...
        // Define neighbors for use throughout the function
        const neighbors = [
            { dx: -1, dy: 0 }, // left
            { dx: 1, dy: 0 },  // right
            { dx: 0, dy: -1 }, // up
        ...
        // If lava gets extremely cold or under pressure, turn to stone
        if (grid[y][x].temperature < 700) {
            // Convert to stone
            grid[y][x] = {
                type: 'stone',
        ...
        // Check for pressure (surrounded by solid materials)
        let solidNeighbors = 0;
        for (const dir of neighbors) {
            const nx = x + dir.dx;
            const ny = y + dir.dy;

            if (isInBounds(nx, ny) && grid[ny][nx] && grid[ny][nx].isSolid) {
                solidNeighbors++;
            }
        }

        // If under high pressure, turn to stone
        if (solidNeighbors >= 3 && Math.random() < 0.05) {
            grid[y][x] = {
                type: 'stone',
                color: '#808080',
            ...
            if (grid[ny][nx] && grid[ny][nx].temperature !== undefined) {
                ...
                switch (grid[ny][nx].type) {
                    ...
                    default:
                        ...
                        if (grid[ny][nx].type !== 'steel') { // Always melt through materials
                            ...
                            if (isMetal) {
                                grid[y][x].temperature -= 10; // Minimal cooling
                            } else if (isDense) {
                                grid[y][x].temperature -= 5; // Minimal cooling
                            } else {
                                grid[y][x].temperature -= 2; // Minimal cooling
                            }
                        }
                        break;
                }
